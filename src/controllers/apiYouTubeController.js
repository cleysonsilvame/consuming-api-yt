...
const getBroadcast = async (req, res) => {
  return youtube.liveBroadcasts
    ...
    .then((response) => {
      ...
      if (res) res.json(liveChatIds);
      return liveChatIds.lives;
    })
    .catch((err) => {
      if (res) res.status(err.code).send(err.errors);
      console.log(`Erro ao buscar Lives ${err.message}`);
      return { error: 'ERRO: ' + err.message };
    });
};

const getCommentsInternal = async (
  codeLive,
  pageToken = '',
  comentarios_array = [],
  req,
  ...
  return youtube.liveChatMessages
    ...
    .then((response) => {
      ...
      if (commentsInfo.totalResults === 0) return comentarios_array;
...
const getComments = async (req, res) => {
  let codeLive = req.params.codelive;
